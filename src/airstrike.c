...
PSP_MODULE_INFO("airstrike", 0, 1, 1);
...
static int general_setup(void)
{
  ...
  if((i = SDL_Init(SDL_INIT_VIDEO | SDL_INIT_JOYSTICK | SDL_INIT_AUDIO ))<0)
    {
      fprintf(stderr,"Error initializing SDL.SDL=<%s>\n", SDL_GetError());
      return -1;
    }
  ...
  sprite_global.display = SDL_SetVideoMode(screen_w, screen_h, 16, SDL_SWSURFACE);
...
void players_setup(void)
{
  ...
  player_startpos[0][0] = screen_w-50;
  player_startpos[0][1] = screen_h/2;
  player_startpos[1][0] = 50;
  player_startpos[1][1] = screen_h/2;
...
void objects_setup(void)
{
  ...
  for (i=0;i<2;i++)
    {
      ...
      sprite_group_insert(mech_group,sp);
      sprite_set_pos(sp,rand() % 220 + 50,rand() % 50 + 25);
    }
  
  sp = sprite_create(&cloud);
  sprite_group_insert(foreground_group,sp);
  sprite_set_pos(sp,280,70);
  v[0] = 15; v[1] = 0;
  sprite_set_vel(sp,v);
  ...
  sprite_group_insert(mech_group,sp);
  sprite_set_pos(sp,rand()%320,rand()%240);
  ...
  for (i=0;i<MAXPLAYERS;i++)
      ...
      if (!sprite_isvalid(&player_controller[i]->target))
	{
	  ...
	  if (player_points[i] <= 0)
	    {
	      if (i == 0)
		sprintf(cbuf,"And the winner is %s\n by %i point(s)",
...
int main(int argc, char *argv[])
{
  ...
  if (argc > 1)
    {
      fprintf(stderr,"All options are currently given in the config files, located at $HOME/.airstrikerc and $PWD/airstrikerc. Quitting.\n");
      exit(EXIT_SUCCESS);
    }
  res = general_setup() == 0;
  assert(res);
